...
describe('getRoast', () => {
  ...
  it('uses max_completion_tokens instead of deprecated max_tokens', async () => {
    const create = vi.fn(async () => ({ choices: [{ message: { content: 'kena mental tapi sayang' } }] }))

    await getRoast({ username: 'fauzan', vibe: 'quote' }, { client: fakeClient(create) })

    expect(create).toHaveBeenCalledWith(expect.objectContaining({ max_completion_tokens: 240 }))
    expect(create).toHaveBeenCalledWith(expect.not.objectContaining({ max_tokens: expect.anything() }))
  })
  ...
  it('round robins to the next configured model when a request fails', async () => {
    ...
    const create = vi
      ...
      .mockResolvedValueOnce({ choices: [{ message: { content: 'akhirnya nyala 🔥' } }] })

    const roast = await getRoast({ username: 'fauzan', vibe: 'quote' }, { client: fakeClient(create) })
